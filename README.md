...
- **Text blocks** — any visible text (headings, paragraphs, buttons, links, list items, captions, table cells). Edit inline in the preview or via the side-panel textarea. Per block: text colour, background colour, font family (Sans / Serif / Display / Mono / Rounded), and — for `<a>` elements — the `href` link.
...
Every row also has **move up / move down / delete** controls so you can reorder elements among their siblings or remove them outright.
...
- **Per-block style controls** — text colour, background colour, border colour, border radius, padding, font family, icon colour, link `href`.
- **Reorder & delete** — move up / move down / delete buttons on every row; move actions are undoable, delete is confirmed.
