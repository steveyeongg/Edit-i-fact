# Edit-i-fact (HTML Artifact Editor)

**Edit HTML artifacts like PowerPoint — without touching code.**

![Edit-i-fact editor screenshot](docs/demo/hero.png)

A browser-based visual editor for `.html` files generated by AI tools (Claude artifacts, ChatGPT, Cursor). Upload a slide deck, landing page, or dashboard — then edit text, resize typography, swap images, recolour shapes, restyle icons, and change borders, all without touching code.
...
- **Text blocks** — any visible text (headings, paragraphs, buttons, links, list items, captions, table cells). Edit inline in the preview or via the side-panel textarea. Per block: text colour, background colour, font family (Sans / Serif / Display / Mono / Rounded), font size, and — for `<a>` elements — the `href` link.
- **Shapes & boxes** — any container with a visible background, border, or rounded corners (cards, KPI tiles, hero sections). Edit fill, border colour, corner radius, and inner padding.
...
Every row also has **move up / move down / delete** controls so you can reorder elements among their siblings or remove them outright. Deleted rows can be restored with Undo.
...
- **Per-block style controls** — text colour, background colour, border colour, border radius, padding, font family, font size, icon colour, link `href`.
- **Reorder & delete** — move up / move down / delete buttons on every row; move and delete actions are undoable, delete is confirmed.
...
- **Undo / redo** — coalesced keystrokes (⌘Z / ⌘⇧Z), covering text, colours, borders, radius, font size, image changes, move actions, and delete / restore.
...
Edits in either surface flow through one of `setText` / `setColor` / `setBgColor` / `setBorderColor` / `setBorderRadius` / `setPadding` / `setFontFamily` / `setFontSize` / `setHref` / `setIconColor` / `setImageSrc` / `setImageAlt` / `deleteItem` / `restoreItem`, which mutate the source document and mirror the change to whichever surface didn't originate the edit. The `data-edit-id` attributes are stripped on download, so the output is byte-clean.
